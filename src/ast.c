...
visit_statement(Ty *ty, Stmt *s, Scope *scope, VisitorSet const *hooks)
{
        ...
        case STATEMENT_MULTI:
                for (usize i = 0; i < s->statements.count; ++i) {
        ...
        case STATEMENT_WHILE:
                for (int i = 0; i < s->_while.parts.count; ++i) {
                        struct condpart *p = s->_while.parts.items[i];
                        V(p->e);
                        VP(p->target);
                }
                SUB(false, "(while)", VS(s->_while.block));
                break;
        case STATEMENT_IF:
                if (s->_if.neg) {
                        VS(s->_if.then);
                        SUB(false, "(if-not)",
                                for (int i = 0; i < s->_if.parts.count; ++i) {
                                        struct condpart *p = s->_if.parts.items[i];
                                        VP(p->target);
                                        V(p->e);
                                }
                                VS(s->_if._else);
...
visit_expression(Ty *ty, Expr *e, Scope *scope, VisitorSet const *hooks)
{
        ...
        case EXPRESSION_FUNCTION:
                ...
                for (usize i = 0; i < vN(e->type_params); ++i) {
