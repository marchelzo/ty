...
{
        if (ip != &JIT || frame == NULL) {
                return ip;
        }

        char const *entry = jit_of(&frame->f);
        char const *bc = code_of(&frame->f);
        if (entry == NULL || entry == (void *)0xFA57) {
                return bc + 1;
        }

        JitCode const *code  = (void const *)(entry - sizeof *code);
        JitSite const *sites = (void const *)(entry + code->table);

        if (frame->jit_pc != NULL) {
                uptr pc = *frame->jit_pc - (uptr)entry;
                u32 lo = 0;
                u32 hi = code->nsites;
                while (lo < hi) {
                        u32 mid = lo + (hi - lo) / 2;
                ...
                if (lo < code->nsites && sites[lo].pc == pc) {
                        return bc + sites[lo].bc;
                }
        } else if (frame->jit_resume > 0 && frame->jit_resume <= code->nresume) {
                u32 const *resumes = (void const *)(sites + code->nsites);
                return bc + resumes[frame->jit_resume - 1];
        }
...
#define OFF_TY_IP     offsetof(Ty, ip)
...
static char const *jit_opt_names[JIT_OPT_COUNT] = {
        ...
        [JIT_OPT_SELF_CALL]              = "emit direct self call",
...
jit_stats_report(Ty *ty, FILE *out)
{
        ...
        bool have_optimizations = false;
        for (int i = 0; i < JIT_OPT_COUNT; ++i) {
                have_optimizations |= jit_opt_sites[i] != 0;
        }
        if (have_optimizations) {
                fprintf(out, "%s======= JIT optimizations =======%s\n\n",
                        PTERM(95), PTERM(0));
                fprintf(out, "   %s%-32s %12s  %12s%s\n",
                        PTERM(90), "Plan", "Sites", "Units", PTERM(0));
                for (int i = 0; i < JIT_OPT_COUNT; ++i) {
                        if (jit_opt_sites[i] == 0) {
                                continue;
                        }
                        fprintf(out, "   %-32s %12llu  %12llu\n",
                                jit_opt_names[i],
                                (unsigned long long)jit_opt_sites[i],
                                (unsigned long long)jit_opt_units[i]);
                }
                fputc('\n', out);
        }
        ...
        u64 total_fast = jit_stats.member_fast + jit_stats.member_set_fast
                       ...
                       + jit_stats.self_tail_fast + jit_stats.numeric_pow_fast;
        u64 total_slow = 0;
        ...
        total_slow += jit_stats.direct_math_slow + jit_stats.direct_max_slow
                    ...
                    + jit_stats.self_tail_slow + jit_stats.numeric_pow_slow;
        ...
        if (total_fast + total_slow > 0) {
                ...
                fast_path_row(out, "direct sin/cos",
                              jit_stats.direct_math_fast,
                              jit_stats.direct_math_slow);
                fast_path_row(out, "direct max", jit_stats.direct_max_fast,
                              jit_stats.direct_max_slow);
                fast_path_row(out, "simple constructor",
                              jit_stats.simple_ctor_fast,
                              jit_stats.simple_ctor_slow);
                fast_path_row(out, "linked global call",
                              jit_stats.linked_global_fast,
                              jit_stats.linked_global_slow);
                fast_path_row(out, "global JIT call",
                              jit_stats.global_jit_call_fast,
                              jit_stats.global_jit_call_slow);
                fast_path_row(out, "self call", jit_stats.self_call_fast,
                              jit_stats.self_call_slow);
                fast_path_row(out, "self tail loop",
                              jit_stats.self_tail_fast,
                              jit_stats.self_tail_slow);
                fast_path_row(out, "numeric pow",
...
#define JIT_OPT_APPLIED(ctx, opt) (++jit_opt_sites[(opt)])
#define JIT_OPT_APPLIED_N(ctx, opt, n) do { \
        ++jit_opt_sites[(opt)];             \
        jit_opt_units[(opt)] += (n);        \
} while (0)
#define EMIT_FUSED_TICK(bc_ip) do { \
        ctx->ip = StepInstruction(bc_ip); \
        bc_emit_profiler_tick_at(ctx, (bc_ip)); \
} while (0)
...
#define JIT_OPT_APPLIED_N(ctx, opt, n) ((void)0)
#define EMIT_FUSED_TICK(bc_ip) (ctx->ip = StepInstruction(bc_ip))
#endif
...
{
        if (v->type != VALUE_STRING || table_size <= 0) {
                return -1;
        }
        ...
        for (;;) {
                i32 id;
                memcpy(&id, table + bucket * 2 * sizeof (i32), sizeof id);
                if (id == -1) {
                        return -1;
                }
...
static bool
jit_simple_ctor_plan(Ty *ty, int class_id, int argc, u64 *packed,
                     bool *nil_guard)
{
        Class *class = class_get(ty, class_id);
        if (!class->really_final) {
                return false;
        }

        Value *ctor = class_ctor(ty, class_id);
        if (ctor == NULL || ctor->type != VALUE_FUNCTION
            || argc != param_count_of(ctor) || argc <= 0 || argc > 8
            || rest_idx_of(ctor) != -1 || kwargs_idx_of(ctor) != -1) {
                return false;
        }

        char const *ip = code_of(ctor);
        char const *end = ip + code_size_of(ctor);
        int self_local = argc;
        u64 map = 0;
        unsigned seen = 0;
        bool defaults = false;
        ...
        while (ip < end) {
                ...
                if (op == INSTR_LOAD_LOCAL) {
                        i32 local;
                        if (ip + sizeof local > end) {
                                return false;
                        }
                        memcpy(&local, ip, sizeof local);
                        ip += sizeof local;
#ifndef TY_NO_LOG
                        ip += strlen(ip) + 1;
                        if (ip > end) {
...
#endif
                        if (local < 0 || local >= argc) {
                                defaults = true;
                                continue;
                        }

                        char const *p = ip;
                        if (p >= end || (u8)*p++ != INSTR_LOAD_LOCAL) {
                                defaults = true;
                                continue;
                        }
                        i32 self;
                        if (p + sizeof self > end) {
                                return false;
                        }
                        memcpy(&self, p, sizeof self);
                        p += sizeof self;
#ifndef TY_NO_LOG
                        p += strlen(p) + 1;
                        if (p > end) {
                        ...
                        if (self != self_local
                            || p >= end
                            || (u8)*p++ != INSTR_TARGET_MEMBER) {
                                defaults = true;
                                continue;
                        }

                        i32 member;
                        if (p + sizeof member > end) {
                                return false;
                        }
                        memcpy(&member, p, sizeof member);
                        p += sizeof member;
                        if (member < 0 || member >= vN(class->offsets_w)) {
                                return false;
                        }
                        u16 off = v__(class->offsets_w, member);
                        if ((off >> OFF_SHIFT) != OFF_FIELD
                            || (off & OFF_MASK) > UINT8_MAX
                            || p >= end || (u8)*p++ != INSTR_ASSIGN) {
                                return false;
                        }
                        if (p < end && (u8)*p == INSTR_POP) {
                                ++p;
                        }
...
static int
jit_rt_simple_ctor(Ty *ty, Value *result, int class_id, int argc,
                   u64 packed, int nil_guard)
{
        Value *callee = result + argc;
        if (callee->type != VALUE_CLASS || callee->class != class_id) {
                STAT(simple_ctor_slow);
                return 0;
        }
        if (nil_guard) {
                for (int i = 0; i < argc; ++i) {
                        if (result[i].type == VALUE_NIL) {
                                STAT(simple_ctor_slow);
                                return 0;
                        }
                }
        ...
        for (int i = 0; i < argc; ++i) {
                object.object->slots[(packed >> (i * 8)) & UINT8_MAX]
                        = result[i];
        }
        object.object->init = true;
        *result = object;
...
static int
jit_rt_numeric_pow(Value *base, Value const *exponent)
{
        Value a = *base;
        Value b = *exponent;
        if ((a.type != VALUE_INTEGER && a.type != VALUE_REAL)
            || (b.type != VALUE_INTEGER && b.type != VALUE_REAL)) {
                STAT(numeric_pow_slow);
                return false;
        }
        double x = a.type == VALUE_INTEGER ? (double)a.z : a.real;
        double y = b.type == VALUE_INTEGER ? (double)b.z : b.real;
        double result = pow(x, y);
        *base = a.type == VALUE_INTEGER && b.type == VALUE_INTEGER
...
static int
jit_rt_double_math(Value *result, Value const *arg, int op)
{
        if (arg->type != VALUE_REAL) {
                STAT(direct_math_slow);
                return 0;
        }
        *result = REAL(op == 1 ? sin(arg->real) : cos(arg->real));
...
{
        if (a->type != VALUE_REAL || b->type != VALUE_REAL) {
                STAT(direct_max_slow);
                return 0;
        }
        int cmp = b->real < a->real ? -1 : b->real != a->real;
        *result = cmp > 0 ? *b : *a;
...
static void
jit_rt_tail_loop(Value *locals, Value const *arguments, int param_count,
                 int bound)
{
        memmove(locals, arguments, param_count * sizeof *locals);
        for (int i = param_count; i < bound; ++i) {
                locals[i] = NIL;
        }
        STAT(self_tail_fast);
}
...
static void
bc_emit_call(JitCtx *ctx, int reg)
{
        bc_raw_reset(ctx);
...
bc_emit_runtime_call(JitCtx *ctx, int reg)
{
        ...
        bc_raw_reset(ctx);
        jit_emit_runtime_call_reg(&ctx->asm, reg);
        jit_emit_label(&ctx->asm, label);
        xvP(ctx->sites, ((JitSite) {
                .pc = label,
                .bc = ctx->ip - code_of(ctx->func)
        }));
}
...
{
        int slot = bc_raw_slot(ctx, local);
        if (slot < 0 || ctx->raw_locals[slot].class_id != class_id) {
                return -1;
        }
        ...
        int type = class_id == CLASS_INT ? VALUE_INTEGER
                 : class_id == CLASS_FLOAT ? VALUE_REAL : VALUE_BOOLEAN;
        jit_emit_ldrb(
                asm, BC_S1, BC_LOC,
                local * VALUE_SIZE + VAL_OFF_TYPE
        );
        jit_emit_cmp_ri(asm, BC_S1, type);
        jit_emit_branch_ne(asm, lbl_slow);
        jit_emit_ldr64(
                asm, ctx->raw_locals[slot].reg, BC_LOC,
                local * VALUE_SIZE + VAL_OFF_Z
        );
        ...
        return ctx->raw_locals[slot].reg;
}
...
bc_prescan(JitCtx *ctx, char const *code, int code_size)
{
        ...
        /* Explicit tail calls form an implicit backedge to function entry. */
...
{
        if (vN(ty->st->frames) == 0) {
                STAT(self_call_slow);
                return 0;
        }

        Value callee = vvL(ty->st->frames)->f;
        if (fn->type != callee.type || fn->info != callee.info
            || fn->env != callee.env || argc != param_count_of(&callee)) {
                STAT(self_call_slow);
                return 0;
        }
        ...
        vN(STACK) = (out + argc) - items;
        jit_fast_frame(ty, &callee, NULL, argc);
        vm_trampoline_linked(ty, jit, callee.env);
        STAT(self_call_fast);
        return 1 + (items != vv(STACK));
}

static int
jit_rt_fast_self_tail(Ty *ty, Value *arguments, Value *fn, int argc)
{
        if (vN(ty->st->frames) == 0) {
                STAT(self_tail_slow);
                return 0;
...
jit_rt_linked_global_call(Ty *ty, int gi, int argc, JitFn *expected)
{
        ...
        if (fn->type != VALUE_FUNCTION || jit_of(fn) != expected) {
                STAT(linked_global_slow);
                return 0;
        }
        Value *items = vv(ty->st->stack);
        jit_fast_frame(ty, fn, NULL, argc);
        vm_trampoline_linked(ty, expected, fn->env);
...
jit_rt_concat_strings(Ty *ty, Value *result, Value *base, int n)
{
        ...
        *v_(STACK, result_idx) = STRING(str, total);
}
...
static void
bc_emit_global_incdec(JitCtx *ctx, void *helper)
{
        dasm_State **asm = &ctx->asm;
...
{
        dasm_State **asm = &ctx->asm;
        int off = local * VALUE_SIZE;
        jit_emit_ldrb(asm, scratch, BC_LOC, off + VAL_OFF_TYPE);
        jit_emit_cmp_ri(asm, scratch, VALUE_INTEGER);
        jit_emit_branch_ne(asm, lbl_slow);
        return -1;
}
                       ...
                       u8 op, int lbl_target)
{
        ...
        if (right >= 0) {
                ...
                jit_emit_load_imm(asm, BC_S0, 0);
                jit_emit_stp64(asm, BC_S0, BC_S0, BC_OPS, right_off);
                jit_emit_stp64(asm, BC_S0, BC_S0, BC_OPS, right_off + 16);
                jit_emit_load_imm(asm, BC_S0, VALUE_INTEGER);
                jit_emit_strb(asm, BC_S0, BC_OPS, right_off + VAL_OFF_TYPE);
                jit_emit_load_imm(asm, BC_S0, immediate);
                jit_emit_str64(asm, BC_S0, BC_OPS, right_off + VAL_OFF_Z);
        }
        jit_emit_mov(asm, BC_A0, BC_TY);
...
{
        char const *p = *ip;
        if (p >= end || (u8)*p++ != op || end - p < (isize)sizeof *local) {
                return false;
        }
        ...
        p += sizeof *local;
#ifndef TY_NO_LOG
        char const *nul = memchr(p, '\0', (usize)(end - p));
        if (nul == NULL) {
                return false;
        }
        p = nul + 1;
...
static void
bc_emit_swap_fallback(JitCtx *ctx, char const *ip, char const *end)
{
        dasm_State **asm = &ctx->asm;
                        ...
                        char const **ip, int load_off, int array_local)
{
        ...
        if (!bc_read_local_operand(&p, end, INSTR_LOAD_LOCAL, &i)
            || !bc_match_op(&p, end, INSTR_SUBSCRIPT)
            || !bc_read_local_operand(&p, end, INSTR_ASSIGN_LOCAL, &tmp)
            || !bc_read_local_operand(&p, end, INSTR_LOAD_LOCAL, &array2)
            || !bc_read_local_operand(&p, end, INSTR_LOAD_LOCAL, &j)
            || !bc_match_op(&p, end, INSTR_SUBSCRIPT)
            || !bc_read_local_operand(&p, end, INSTR_LOAD_LOCAL, &array3)
            || !bc_read_local_operand(&p, end, INSTR_LOAD_LOCAL, &i2)
            || !bc_match_assign_subscript(&p, end)
            || !bc_match_op(&p, end, INSTR_POP)
            || !bc_read_local_operand(&p, end, INSTR_LOAD_LOCAL, &tmp2)
            || !bc_read_local_operand(&p, end, INSTR_LOAD_LOCAL, &array4)
            || !bc_read_local_operand(&p, end, INSTR_LOAD_LOCAL, &j2)
            || !bc_match_assign_subscript(&p, end)
            || !bc_match_op(&p, end, INSTR_POP)) {
                return false;
        }

        char const *after_swap = p;
        bool fold_cursors = false;
        if (end - p >= 2 && (u8)p[0] == INSTR_INT8 && (i8)p[1] == 1) {
                p += 2;
                int target_i, target_j;
                if (bc_read_local_operand(
                            &p, end, INSTR_TARGET_LOCAL, &target_i
                    )
                    && bc_match_op(&p, end, INSTR_MUT_ADD)
                    && bc_match_op(&p, end, INSTR_POP)
                    && end - p >= 2
                    && (u8)p[0] == INSTR_INT8
                    && (i8)p[1] == 1) {
                        ...
                        fold_cursors = bc_read_local_operand(
                                &p, end, INSTR_TARGET_LOCAL, &target_j
                        ) && bc_match_op(&p, end, INSTR_MUT_SUB)
                          && bc_match_op(&p, end, INSTR_POP)
        ...
        if (array2 != array_local || array3 != array_local
            || array4 != array_local || i2 != i || j2 != j || tmp2 != tmp
            || array_local < 0 || array_local >= ctx->bound
            || i < 0 || i >= ctx->bound || j < 0 || j >= ctx->bound) {
                return false;
        }
        for (char const *q = *ip; q < p; ++q) {
                if (bc_find_label(ctx, (int)(q - code)) >= 0) {
                        return false;
                }
        }

#ifdef TY_PROFILER
        bc_emit_profiler_ticks_between(ctx, code, load_off, p);
#endif
        dasm_State **asm = &ctx->asm;
        int lbl_slow = bc_next_label(ctx);
        int lbl_done = bc_next_label(ctx);
        int array_off = array_local * VALUE_SIZE;
        int i_off = i * VALUE_SIZE;
        int j_off = j * VALUE_SIZE;
        int temporary_off = OP_OFF(ctx->sp);

        jit_emit_ldrb(asm, BC_S0, BC_LOC, array_off + VAL_OFF_TYPE);
        jit_emit_cmp_ri(asm, BC_S0, VALUE_ARRAY);
        jit_emit_branch_ne(asm, lbl_slow);
        jit_emit_ldrb(asm, BC_S0, BC_LOC, i_off + VAL_OFF_TYPE);
        jit_emit_cmp_ri(asm, BC_S0, VALUE_INTEGER);
        jit_emit_branch_ne(asm, lbl_slow);
        jit_emit_ldrb(asm, BC_S0, BC_LOC, j_off + VAL_OFF_TYPE);
        jit_emit_cmp_ri(asm, BC_S0, VALUE_INTEGER);
        jit_emit_branch_ne(asm, lbl_slow);
        jit_emit_ldr64(asm, BC_S0, BC_LOC, i_off + VAL_OFF_Z);
        jit_emit_ldr64(asm, BC_S3, BC_LOC, j_off + VAL_OFF_Z);
        ...
        jit_emit_shl(asm, BC_S3, BC_S3, BC_S2);
        jit_emit_add(asm, BC_RET, BC_S1, BC_S0);
        jit_emit_add(asm, BC_S3, BC_S1, BC_S3);
        jit_emit_add_imm(asm, BC_S2, BC_OPS, temporary_off);
        bc_copy_value(ctx, BC_S2, 0, BC_RET, 0);
        bc_copy_value(ctx, BC_RET, 0, BC_S3, 0);
        bc_copy_value(ctx, BC_S3, 0, BC_S2, 0);
        if (fold_cursors) {
                jit_emit_ldr64(asm, BC_S0, BC_LOC, i_off + VAL_OFF_Z);
                jit_emit_add_imm(asm, BC_S0, BC_S0, 1);
                jit_emit_str64(asm, BC_S0, BC_LOC, i_off + VAL_OFF_Z);
                jit_emit_ldr64(asm, BC_S0, BC_LOC, j_off + VAL_OFF_Z);
                jit_emit_add_imm(asm, BC_S0, BC_S0, -1);
                jit_emit_str64(asm, BC_S0, BC_LOC, j_off + VAL_OFF_Z);
        }
        jit_emit_jump(asm, lbl_done);

        jit_emit_label(asm, lbl_slow);
        bc_emit_swap_fallback(ctx, code + load_off, after_swap);
        if (fold_cursors) {
                char const *mut_i = NULL;
                char const *mut_j = NULL;
                for (char const *q = after_swap; q < p; q = StepInstruction(q)) {
                        if ((u8)*q == INSTR_MUT_ADD) {
                                mut_i = StepInstruction(q);
                        } else if ((u8)*q == INSTR_MUT_SUB) {
                                mut_j = StepInstruction(q);
                        }
                }
                ctx->ip = mut_i;
                jit_emit_load_imm(asm, BC_S0, 0);
                jit_emit_stp64(asm, BC_S0, BC_S0, BC_OPS, temporary_off);
                jit_emit_stp64(
                        asm, BC_S0, BC_S0, BC_OPS, temporary_off + 16
                );
                jit_emit_load_imm(asm, BC_S0, VALUE_INTEGER);
                jit_emit_strb(
                        asm, BC_S0, BC_OPS, temporary_off + VAL_OFF_TYPE
                );
                jit_emit_load_imm(asm, BC_S0, 1);
                jit_emit_str64(
                        asm, BC_S0, BC_OPS, temporary_off + VAL_OFF_Z
                );
                jit_emit_mov(asm, BC_A0, BC_TY);
                jit_emit_add_imm(asm, BC_A1, BC_LOC, i_off);
                jit_emit_add_imm(asm, BC_A2, BC_OPS, temporary_off);
                              ...
                              int load_off, int array_local)
{
        ...
        jit_emit_branch_ne(asm, lbl_slow);
        if (index_local >= 0) {
                int local_off = index_local * VALUE_SIZE;
        ...
        jit_emit_stp64(asm, BC_S0, BC_S2, BC_LOC, destination_off);
        jit_emit_ldp64(asm, BC_S0, BC_S2, BC_S1, 16);
        jit_emit_stp64(asm, BC_S0, BC_S2, BC_LOC, destination_off + 16);
        jit_emit_jump(asm, lbl_done);
                             ...
                             int load_off, int array_local)
{
...
#endif
        ctx->ip = StepInstruction(code + assign_off);
        dasm_State **asm = &ctx->asm;
        ...
        jit_emit_shl(asm, BC_S0, BC_S0, BC_S2);
        jit_emit_add(asm, BC_S1, BC_S1, BC_S0);
        jit_emit_ldp64(asm, BC_S0, BC_S2, BC_OPS, value_off);
        jit_emit_stp64(asm, BC_S0, BC_S2, BC_S1, 0);
        jit_emit_ldp64(asm, BC_S0, BC_S2, BC_OPS, value_off + 16);
                       ...
                       int load_off, int array_local)
{
        ...
        jit_emit_add(asm, BC_S1, BC_S1, BC_S0);
        jit_emit_ldp64(asm, BC_S0, BC_S2, BC_S1, 0);
        jit_emit_stp64(asm, BC_S0, BC_S2, BC_OPS, result_off);
        jit_emit_ldp64(asm, BC_S0, BC_S2, BC_S1, 16);
        jit_emit_stp64(asm, BC_S0, BC_S2, BC_OPS, result_off + 16);
        jit_emit_jump(asm, lbl_done);
                   ...
                   char const **ip, Symbol **locals, int dup_off)
{
        ...
        jit_emit_cmp_ri(asm, BC_S0, VALUE_INTEGER);
        jit_emit_branch_ne(asm, lbl_slow);
        jit_emit_ldrb(asm, BC_S0, BC_OPS, cursor_off + VAL_OFF_TYPE);
        jit_emit_cmp_ri(asm, BC_S0, VALUE_INTEGER);
        jit_emit_branch_ne(asm, lbl_slow);
        EMIT_STAT(jit_rt_stat_jcmp_int);
        jit_emit_ldr64(asm, BC_S0, BC_OPS, cursor_off + VAL_OFF_Z);
        jit_emit_ldr64(asm, BC_S1, BC_OPS, bound_off + VAL_OFF_Z);
                       ...
                       char const **ip, Symbol **locals, int load_off, int local)
{
        ...
        jit_emit_branch_ne(asm, lbl_slow);
        jit_emit_ldrb(asm, BC_S0, BC_LOC, index_off + VAL_OFF_TYPE);
        jit_emit_cmp_ri(asm, BC_S0, VALUE_INTEGER);
        jit_emit_branch_ne(asm, lbl_slow);
        jit_emit_ldr64(asm, BC_S0, BC_LOC, index_off + VAL_OFF_Z);
        jit_emit_cmp_ri(asm, BC_S0, 0);
        jit_emit_branch_lt(asm, lbl_slow);
        if (container_class->i == CLASS_ARRAY) {
                jit_emit_ldr64(asm, BC_S1, BC_OPS, result_off + VAL_OFF_Z);
                jit_emit_ldr64(asm, BC_S2, BC_S1, OFF_VEC_LEN);
                jit_emit_cmp_lt(asm, BC_S2, BC_S0, BC_S2);
                jit_emit_cbz(asm, BC_S2, lbl_slow);
                jit_emit_ldr64(asm, BC_S1, BC_S1, OFF_VEC_DATA);
                    ...
                    int target, u8 op, int class_id)
{
        ...
        if (class_id == CLASS_INT) {
                if (op == INSTR_MUT_ADD) {
                        ...
                        jit_emit_sub(asm, left, left, right);
                } else {
...
bc_emit_local_int_imm_mut_pop(JitCtx *ctx, int target, u8 op, imax value)
{
        ...
        if (op == INSTR_MUT_ADD) {
                ...
                jit_emit_sub(asm, value_reg, value_reg, BC_S1);
        } else {
        ...
        jit_emit_label(asm, lbl_slow);
        jit_emit_load_imm(asm, BC_S0, 0);
        jit_emit_stp64(asm, BC_S0, BC_S0, BC_OPS, scratch_off);
        jit_emit_stp64(asm, BC_S0, BC_S0, BC_OPS, scratch_off + 16);
        jit_emit_load_imm(asm, BC_S0, VALUE_INTEGER);
        jit_emit_strb(asm, BC_S0, BC_OPS, scratch_off + VAL_OFF_TYPE);
        jit_emit_load_imm(asm, BC_S0, value);
...
static void
bc_emit_arith_ex(JitCtx *ctx, void *helper, int fallback)
{
        dasm_State **asm = &ctx->asm;
        ...
        bool mixed_a_int = a_cls != NULL && a_cls->i == CLASS_INT
                        && b_cls != NULL && b_cls->i == CLASS_FLOAT;
        bool mixed_b_int = a_cls != NULL && a_cls->i == CLASS_FLOAT
                        && b_cls != NULL && b_cls->i == CLASS_INT;
        bool mixed_float = mixed_a_int || mixed_b_int;

        bool float_arith = (try_float || mixed_float)
                        && (
        ...
        int lbl_slow = (fallback >= 0) ? fallback : bc_next_label(ctx);
        int lbl_done = bc_next_label(ctx);
        ...
        if (float_arith) {
                ...
                if (mixed_float) {
                        int arith_op = helper == (void *)jit_rt_add ? 0
                                     : helper == (void *)jit_rt_sub ? 1
                                     : helper == (void *)jit_rt_mul ? 2 : 3;
                        jit_emit_farith_mixed(asm, BC_OPS,
                                             a_off + VAL_OFF_Z, b_off + VAL_OFF_Z,
                                             mixed_a_int, arith_op);
                        if (mixed_a_int) {
                                jit_emit_load_imm(asm, BC_S0, VALUE_REAL);
                                jit_emit_strb(asm, BC_S0, BC_OPS, a_off + VAL_OFF_TYPE);
                        }
                } else if (helper == (void *)jit_rt_add) {
...
bc_emit_trampoline_signal(JitCtx *ctx, int status, int idx)
{
        ...
        ctx->resume_offsets[idx - 1] = ctx->ip - code_of(ctx->func);
...
bc_emit_inline_comparison(JitCtx *ctx, u8 op, int left, int right, int lbl_slow)
{
        ...
        jit_emit_cmp_ri(asm, BC_S0, VALUE_INTEGER);
        jit_emit_branch_ne(asm, lbl_slow);
        bc_emit_inline_mixed_comparison(
                ctx, op, left, right, false, lbl_slow
        );
        ...
        jit_emit_branch_ne(asm, lbl_slow);
        bc_emit_inline_mixed_comparison(
                ctx, op, left, right, true, lbl_slow
        );
        jit_emit_jump(asm, lbl_write);
                    ...
                    int lbl_slow)
{
        ...
        for (int i = 0; i < plan->count; ++i) {
                ...
                case TY_INLINE_REAL:
                {
                        ...
                        jit_emit_load_imm(asm, BC_S0, VALUE_REAL);
                        jit_emit_strb(asm, BC_S0, BC_OPS, off + VAL_OFF_TYPE);
                        jit_emit_load_imm(asm, BC_S0, bits);
                        jit_emit_str64(asm, BC_S0, BC_OPS, off + VAL_OFF_Z);
                        depth++;
                        break;
                }
                ...
                case TY_INLINE_DIV:
                {
                        ...
                        bc_emit_arith_ex(ctx, (void *)jit_rt_div, lbl_slow);
                        ctx->sp = saved_sp;
                        depth--;
                        break;
                }
                ...
                case TY_INLINE_BRANCH_TRUE:
                        jit_emit_ldr64(
                                asm, BC_S0, BC_OPS,
                                OP_OFF(scratch + depth - 1) + VAL_OFF_Z
                        );
                        jit_emit_cbnz(asm, BC_S0, labels[insn->target]);
                        break;
        ...
        while (ip < end) {
                ...
                CASE(JGE) {
                        ...
                        if (try_float || try_int) {
                                ...
                                if (try_int) {
                                        // Check a.type == VALUE_INTEGER
                                        jit_emit_cmp_ri(asm, BC_S0, VALUE_INTEGER);
                                        jit_emit_branch_ne(asm, try_float ? lbl_float : lbl_slow);
                ...
                CASE(MEMBER_ACCESS) {
                        ...
                        if (!emitted_fast && z >= 0) {
                                ...
                                jit_emit_load_imm(asm, BC_S1, z);
                                jit_emit_cmp_rr(asm, BC_S0, BC_S1);
                                jit_emit_branch_ule(asm, lbl_slow);

                                jit_emit_ldr64(asm, BC_S3, BC_S3, offsets_items_off);
                                jit_emit_load_imm(asm, BC_S1, (i64)(u32)z * sizeof (u16));
                                jit_emit_ldr16_index(asm, BC_S0, BC_S3, BC_S1);
                                jit_emit_mov(asm, BC_S1, BC_S0);

                                jit_emit_load_imm(asm, BC_S3, OFF_SHIFT);
                                jit_emit_shr(asm, BC_S0, BC_S0, BC_S3);
                                jit_emit_cmp_ri(asm, BC_S0, OFF_FIELD);
                                jit_emit_branch_ne(asm, lbl_slow);

                                jit_emit_load_imm(asm, BC_S0, OFF_MASK);
                                jit_emit_and(asm, BC_S1, BC_S1, BC_S0);
                                jit_emit_load_imm(asm, BC_S0, 5);
                                jit_emit_shl(asm, BC_S1, BC_S1, BC_S0);
                                jit_emit_add_imm(asm, BC_S2, BC_S2, OBJ_OFF_SLOTS);
                                jit_emit_add(asm, BC_S2, BC_S2, BC_S1);

                                jit_emit_ldp64(asm, BC_S0, BC_S1, BC_S2, 0);
                                jit_emit_stp64(asm, BC_S0, BC_S1, BC_OPS, obj_off);
                                jit_emit_ldp64(asm, BC_S0, BC_S1, BC_S2, 16);
                                jit_emit_stp64(asm, BC_S0, BC_S1, BC_OPS, obj_off + 16);
                                EMIT_STAT(jit_rt_stat_member_fast);
                                jit_emit_jump(asm, lbl_done);
                ...
                CASE(CALL) {
                        ...
                        JIT_OPT_APPLIED(ctx, JIT_OPT_SELF_CALL);
                        jit_emit_mov(asm, BC_A0, BC_TY);
                        jit_emit_add_imm(asm, BC_A1, BC_OPS, out_off);
                        ...
                        /* Test the return value before reloading the stack: the
                         * x86-64 reload uses the return register as scratch. */
                ...
                CASE(TAIL_CALL) {
                        if (ctx->sp < ctx->param_count) {
                                BAIL("TAIL_CALL operand underflow");
                        }
                         ...
                         * locals and non-parameter locals belong to the old
                         * iteration and must not survive this backedge. */
                        jit_emit_load_imm(asm, BC_CACHE_MASK, 0);
                        bc_raw_reset(ctx);
                        JIT_OPT_APPLIED(ctx, JIT_OPT_SELF_TAIL);
                        ctx->sp = 0;
                                        ...
                                        if (field_off != OFF_NOT_FOUND
                                            && (field_off >> OFF_SHIFT) == OFF_FIELD) {
                                                ...
                                                if (slot_off + 16 <= 504) {
                                                        ...
                                                        bc_emit_deref(ctx, BC_S3, BC_LOC, self_off);
                                                        jit_emit_ldrb(asm, BC_S0, BC_S3, VAL_OFF_TYPE);
                                                        jit_emit_cmp_ri(asm, BC_S0, VALUE_OBJECT);
                                                        jit_emit_branch_ne(asm, lbl_slow);
                                                        jit_emit_ldr32(asm, BC_S0, BC_S3, VAL_OFF_CLASS);
                ...
                CASE(POST_INC) {
                        ...
                        if (ctx->tgt_kind == TGT_LOCAL) {
                                ...
                                bc_emit_global_incdec(ctx, (void *)jit_rt_post_inc);
                        } else if (ctx->tgt_kind == TGT_CAPTURED) {
                                int lbl_slow = bc_next_label(ctx);
                                int lbl_done = bc_next_label(ctx);
                ...
                CASE(POST_DEC) {
                        ...
                        if (ctx->tgt_kind == TGT_LOCAL) {
                                ...
                                bc_emit_global_incdec(ctx, (void *)jit_rt_post_dec);
                        } else if (ctx->tgt_kind == TGT_CAPTURED) {
                                int lbl_slow = bc_next_label(ctx);
                                int lbl_done = bc_next_label(ctx);
                ...
                CASE(PRE_INC) {
                        ...
                        if (ctx->tgt_kind == TGT_LOCAL) {
                                ...
                                bc_emit_global_incdec(ctx, (void *)jit_rt_pre_inc);
                        } else if (ctx->tgt_kind == TGT_CAPTURED) {
                                int lbl_slow = bc_next_label(ctx);
                                int lbl_done = bc_next_label(ctx);
                ...
                CASE(PRE_DEC) {
                        ...
                        if (ctx->tgt_kind == TGT_LOCAL) {
                                ...
                                bc_emit_global_incdec(ctx, (void *)jit_rt_pre_dec);
                        } else if (ctx->tgt_kind == TGT_CAPTURED) {
                                int lbl_slow = bc_next_label(ctx);
                                int lbl_done = bc_next_label(ctx);
...
jit_compile(Ty *ty, Value const *func)
{
        ...
        JitCtx ctx = {
                ...
                .save_sp_top    = -1,
                .self_class_id  = -1,
                .registerize    = getenv("TY_JIT_NO_REGISTERIZE") == NULL,
        };
        ...
        usize table_offset = (final_size + alignof (JitSite) - 1)
                           & ~(alignof (JitSite) - 1);
        usize mapping_size = sizeof (JitCode) + table_offset
                           + vN(ctx.sites) * sizeof (JitSite)
                           + ctx.call_site_count * sizeof (u32);
