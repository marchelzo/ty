...
parse_import(Ty *ty)
{
        ...
        s->import.pub = try_consume(KEYWORD_PUB);
        ...
        while (try_consume('.')) {
                expect(TOKEN_IDENTIFIER);
                sxdf(&module, "/%s", tok()->identifier);
                tok()->tag = TT_MODULE;
                next();
        }

        s->import.module = sclonea(ty, vv(module));
