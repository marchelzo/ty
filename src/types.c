...
#define CompileError(...) do { (CompileError)(ty, MOD_COMPILE_ERR, __VA_ARGS__); } while (0)
#define TypeError(...)    do { (CompileError)(ty, MOD_TYPE_ERR,    __VA_ARGS__); } while (0)
...
inline static bool
IsStrLit(Type const *t0)
{
        return TypeType(t0) == TYPE_STRING;
}

inline static bool
IsBoolLit(Type const *t0)
{
...
{
        return IsUnboundVar(t0) && (t0->variadic || t0->packed);
}
...
static Type *
SliceParams(Ty *ty, ParamVector const *params, imax i, imax j, imax k)
{
        ...
        if (k < 0) {
                ...
                for (int ix = hi; ix >= lo; ix += k) {
                        if (ix >= 0 && ix < n) {
                                avP(t0->types, v_(*params, ix)->type);
                        }
                }
        } else {
                int lo = max(iwrap(i, n), 0);
                int hi = min(iwrap(j, n), n);
                for (int ix = lo; ix < hi; ix += k + !k) {
                        if (ix >= 0 && ix < n) {
                                avP(t0->types, v_(*params, ix)->type);
                        }
                }
...
)
{
        ...
        TLOG("flat():");
        TLOG("    %s", ShowType(&before));
        TLOG(" -> %s", ShowType(&after));
...
Reduce(Ty *ty, Type const *t0)
{
        ...
        case TYPE_FUNCTION:
                ...
                t1->rt = t2;
                if (t1->yields != NULL) {
                        t1->yields = Reduce(ty, t1->yields);
                }
                if (t1->sends != NULL) {
                        t1->sends = Reduce(ty, t1->sends);
                }
...
ParamPackOf(Type *t0)
{
        ...
        case TYPE_VARIABLE:
                if (IsTVar(t0)) {
                        return (t0->id & TV_PACK) ? t0 : NULL;
                } else {
                        return t0->packed ? t0 : NULL;
                }

        case TYPE_SUBSCRIPT:
        case TYPE_SLICE:
                return ParamPackOf(t0->val);
        ...
        case TYPE_LIST:
                for (int i = 0; i < vN(t0->types); ++i) {
                        pack = ParamPackOf(v__(t0->types, i));
                        if (pack != NULL) {
                                return pack;
                        }
                }
        ...
        case TYPE_FUNCTION:
                pack = ParamPackOf(t0->rt);
                if (pack != NULL) {
                        return pack;
                }
                for (int i = 0; i < vN(t0->params); ++i) {
                        pack = ParamPackOf(v_(t0->params, i)->type);
                        if (pack != NULL) {
                                return pack;
                        }
                }
        ...
        case TYPE_ALIAS:
                for (int i = 0; i < vN(t0->args); ++i) {
                        pack = ParamPackOf(v__(t0->args, i));
                        if (pack != NULL) {
                                return pack;
                        }
                }
...
ComputeType(Ty *ty, Type const *t0)
{
        ...
        ((Type *)t0)->val = Reduce(ty, type_from_ty(ty, &val));
...
AlmostSameType(Type const *t0, Type const *t1)
{
        ...
        case PAIR_OF(TYPE_ALIAS):
                ...
                if (
                        (vN(t0->bound) != vN(t1->bound))
                     || (vN(t0->args) != vN(t1->args))
                ) {
                        return false;
                }
...
UnionOf(Ty *ty, Type *t0)
{
        ...
        case TYPE_TUPLE:
        case TYPE_UNION:
                avPv(u->types, t0->types);
                break;

        case TYPE_OBJECT:
                class = &t0->class->def->class;
                for (int i = 0; i < vN(class->fields); ++i) {
                        Expr const *field = v__(class->fields, i);
                        Type *t00 = FieldIdentifier(field)->_type;
                        avP(u->types, t00);
                }
                break;
        ...
        u = (vN(u->types)  > 1) ? Reduce(ty, u)
          : (vN(u->types) == 1) ? v__(u->types, 0)
          : BOTTOM;
...
CountRefs(Type *t0, u32 id)
{
        ...
        case TYPE_FUNCTION:
                ...
                n += CountRefs(t0->yields, id);
                n += CountRefs(t0->sends, id);
                break;
...
type_object(Ty *ty, Class *class)
{
        ...
        if (class->def != NULL) {
                for (int i = 0; i < vN(def->type_params); ++i) {
                        Type *u0 = type_resolve(ty, v__(def->type_params, i));
                        avP(t0->args, u0);
                }
        }
...
GatherFree(Ty *ty, Type *t0, U32Vector *freev, U32Vector *boundv)
{
        ...
        case TYPE_TYPE:
                GatherFree(ty, t0->_type, freev, boundv);
                break;
        ...
        case TYPE_FUNCTION:
                ...
                GatherFree(ty, t0->yields, freev, boundv);
...
Propagate(Ty *ty, Type *t0)
{
        ...
        case TYPE_FUNCTION:
                ...
                t0->concrete &= IsConcrete(t1);
                t0->rt = t1;
                t1 = Propagate(ty, t0->yields);
                if (t1 != t0->yields && !cloned) {
                        t0 = CloneType(ty, t0);
                        cloned = true;
                }
                t0->yields = t1;
                t1 = Propagate(ty, t0->sends);
                if (t1 != t0->sends && !cloned) {
                        t0 = CloneType(ty, t0);
                        cloned = true;
                }
                t0->sends = t1;
...
Occurs(Ty *ty, Type *t0, u32 id, int level)
{
        ...
        case TYPE_TYPE:
                ret = Occurs(ty, t0->_type, id, level);
                break;
        ...
        case TYPE_FUNCTION:
                ...
                if (Occurs(ty, t0->yields, id, level)) {
                        ret = true;
                }
                if (Occurs(ty, t0->sends, id, level)) {
                        ret = true;
                }
                break;
        }
...
NewInst0(Ty *ty, Type *t0, TypeEnv *env)
{
        ...
        case TYPE_FUNCTION:
                ...
                t0->pack = NewInst0(ty, t0->pack, env);
                for (int i = 0; i < vN(t0->constraints); ++i) {
        ...
        case TYPE_TYPE:
                t0->_type = NewInst0(ty, t0->_type, env);
                break;
...
ResolveSuperInstance(Ty *ty, Type *t0, Class const *class)
{
        ...
        while (t0->class != class) {
                XXTLOG("t0=%s", ShowType(t0));
                if (t0->class->def == NULL) {
                        CompileError("can't unify %s before we've seen its definition", ShowType(t0));
                }
        ...
        if (IsObject(t0) && IsObject(t1)) {
                ...
                if (t0->class->i == CLASS_ITERABLE) {
                        if (vN(t0->args) < vN(t1->args)) {
                                UnifyX(ty, INT_TYPE, v__(t1->args, vN(t0->args)), super, false);
                        }
                        if (vN(t1->args) < vN(t0->args)) {
                                UnifyX(ty, v__(t0->args, vN(t1->args)), INT_TYPE, super, false);
                        }
                }
                        ...
                        for (int i = 0; i < vN(t0->types); ++i) {
                                ...
                                if (t2 == NULL) {
                                        if (
                                                (name != NULL)
                                             && s_eq(name, "[]")
                                             && UnifyX(ty, t00, TupleSubscriptType(ty, t1), super, false)
                                        ) {
                                                continue;
                                        }
                                        if (!IsRequired(t0, i)) {
                        ...
                        for (int i = 0; i < vN(t1->types); ++i) {
                                ...
                                if (t2 == NULL) {
                                        if (
                                                (name != NULL)
                                             && s_eq(name, "[]")
                                             && UnifyX(ty, t11, TupleSubscriptType(ty, t0), !super, false)
                                        ) {
                                                continue;
                                        }
                                        if (!IsRequired(t1, i)) {
...
#define OK(...) goto Success;
        ...
        if (TypeType(t0) == TYPE_TYPE && TypeType(t1) == TYPE_TYPE) {
                if (t0->_type == NULL || t1->_type == NULL) {
                        OK("Type[_] with wildcard");
                        goto Success;
                }
                if (UnifyXD(ty, t0->_type, t1->_type, super, check, soft)) {
                        OK("Type[T] inner types unified");
                        goto Success;
                } else {
                        goto Fail;
                }
        ...
        if (PackTypes(t0, t1) == PAIR_OF(TYPE_FUNCTION)) {
                ...
                for (int i = 0; i < vN(t0->params) || i < vN(t1->params); ++i) {
                        ...
                        if (CanBindVariadic(u0)) {
                                TryBind(ty, u0, SliceParams(ty, &t1->params, i, INT_MAX, 1), false);
                                Flatten(ty, t0);
                                break;
                        }

                        if (CanBindVariadic(u1)) {
                                TryBind(ty, u1, SliceParams(ty, &t0->params, i, INT_MAX, 1), false);
                                Flatten(ty, t1);
                                break;
                        }
...
CullConstraints(Ty *ty, Type *t0)
{
        ...
        if (!IsFuncT(t0) || (vN(t0->constraints) == 0)) {
                return t0;
        }
...
{
        Type *cmp0 = CloneType(ty, op_type(ty, OP_CMP));

        if (cmp0 == NULL) {
                return false;
        }

        if (TypeType(cmp0) == TYPE_INTERSECT) {
                CloneVec(cmp0->types);
        }

        Type *u0 = NewVar(ty);

        if (TrySolve2Op(ty, OP_CMP, cmp0, t0, t1, u0, false) != NULL) {
                UnifyX(ty, t2, BOOL_TYPE, true, false);
                return true;
        }
                        ...
                        for (int i = 0; i < IntersectCount(op0); ++i) {
                                ...
                                if (
                                     ...
                                     && type_check(ty, t2_i, op0_i->rt)
                                        ...
                                        right._type = t1_i;
                                        XXTLOG(
                                                "%sTrySolve2Op[InferCall](%s%s%s)%s:",
                                                TERM(93),
                                                TERM(95),
                                                intern_entry(&xD.b_ops, op)->name,
                                                TERM(93),
...
BindConstraint(Ty *ty, Constraint *c)
{
        ...
        case TC_SUB:
                ...
                c->t0 = Reduce(ty, c->t0);
                c->t1 = Reduce(ty, c->t1);
                ok = UnifyX(ty, c->t0, c->t1, false, false)
                  || UnifyX(ty, c->t1, c->t0, true, false);
...
InferCall0(
        ...
        case TYPE_FUNCTION:
                for (int i = 0; i < vN(t0->params); ++i) {
                        Param const *p = v_(t0->params, i);
                        if (p->pack && ParamPackOf(p->type) != NULL) {
                                pack_index = i;
                                break;
                        }
                }
                if (pack_index != -1) {
                        ...
                        for (int ai = pack_index; ai < vN(*args); ++ai) {
                                ...
                                Type *p0   = v_(t00->params, pack_index)->type;
                                Type *arg0 = PossibleArgTypes(ty, args, ai);

                                Type *packed = ParamPackOf(t00);
                                if (packed != NULL) {
                                        packed->packed = false;
                        ...
                        Type *pack_var = ParamPackOf(v_(t0->params, pack_index)->type);

                        if (!UnifyX(ty, pack_var, pack0, true, strict)) {
                                return NULL;
                        }
                ...
                // Propagate yield/send effects to enclosing function
        ...
        case TYPE_VARIABLE:
                if (CanBind(t0)) {
                        ...
                        t1->rt = NewVar(ty);
                                ...
                                for (int i = 0; i < vN(*kwargs); ++i) {
                                        Type *p0 = NewVar(ty);
                                        avP(t1->params, PARAM(v__(*kws, i), p0, false));
                                }
                        CurrentLevel += 1;
...
Inst1(Ty *ty, Type *t0)
{
        ...
        case TYPE_FUNCTION:
                ...
                CloneVec(t1->constraints);
                for (int i = 0; i < vN(t1->params); ++i) {
                        Param *p = v_(t1->params, i);
                        p->type = Inst1(ty, p->type);
                }
                for (int i = 0; i < vN(t1->constraints); ++i) {
                        Constraint *c = v_(t1->constraints, i);
                        switch (c->type) {
                ...
                if (t1->yields != NULL) {
                        t1->yields = Inst1(ty, t1->yields);
                }
                if (t1->sends != NULL) {
                        t1->sends = Inst1(ty, t1->sends);
                }
                break;
        ...
        case TYPE_OBJECT:
                ...
                v00(t1->bound);
                for (int i = 0; i < vN(t1->args); ++i) {
                        *v_(t1->args, i) = Inst1(ty, v__(t1->args, i));
                }
                break;
...
type_member_access_t_(Ty *ty, Type const *t0, char const *name, bool strict, bool setter)
{
        ...
        case TYPE_TAG:
                ...
                if (t0->type == TYPE_CLASS) {
                        c = class_get(ty, CLASS_CLASS);
                } else if (1) {
                        c = class_get(ty, CLASS_TAG);
                } else {
        case TYPE_OBJECT:
                        c = t0->class;
                }
                ...
                if (
                        (t1 == NULL)
                     && (t0->type == TYPE_OBJECT)
                     && ((f = FindMethod(t0->class, "__method_missing__")) != NULL)
                ) {
                        t1 = f->_type;
                }
                if (
                        (t1 == NULL)
                     && (t0->type == TYPE_CLASS)
                     && ((f = FindStatic(t0->class, "__missing__")) != NULL)
                ) {
                        t1 = f->_type->rt;
                }
                if (
                        (t1 == NULL)
                     && (t0->type == TYPE_CLASS)
                     && ((f = FindStatic(t0->class, "__method_missing__")) != NULL)
                ) {
                        t1 = f->_type;
                }
                if (
                ...
                return SolveMemberAccess(ty, t0, ResolveVar(t1));
...
TryUnify:
        t1 = NewVar(ty);
        if (UnifyX(ty, NewRecord(name, t1), (Type *)t0, true, true)) {
                return t1;
        } else {
...
type_member_access(Ty *ty, Expr const *e)
{
        ...
        if (t0 == NULL) {
                if (e->maybe) {
                        ...
                        return type_member_access_t(ty, e->object->_type, e->member->identifier, true);
                }
        }
...
type_method_call_t(Ty *ty, Expr const *e, Type *t0, char const *name)
{
        ...
        case TYPE_INT:
        case TYPE_VARIABLE:
                t1 = type_member_access_t(ty, t0, name, !e->maybe);
                if (t1 != NULL) {
                        return SolveMemberAccess(ty, t0, type_call_t(ty, e, t1));
                }
                break;
...
SubscriptType(Ty *ty, Type *t0, Type *t1)
{
        ...
        case TYPE_OBJECT:
                ...
                case CLASS_STRING: t = STRING_TYPE;    break;
                }
                break;
...
type_check_x_(Ty *ty, Type *t0, Type *t1, bool need)
{
        ...
        if (t0->type == TYPE_TYPE && t1->type == TYPE_TYPE) {
                if (t0->_type == NULL || t1->_type == NULL) {
                        return true;
                }
                return type_check_x(ty, t0->_type, t1->_type, need);
        }
...
type_tuple(Ty *ty, Expr const *e)
{
        ...
        for (int i = 0; i < vN(e->es); ++i) {
                ...
                if (name != NULL && s_eq(name, "*")) {
                        ...
                        Type *et = v__(e->es, i)->_type;
                        AddEntry(t0, et, name, false);
                }
        }
...
type_try_assign(Ty *ty, Expr *e, Type *t0, int flags)
{
        ...
        XXTLOG("try_assign(%s): %s", _show_tflags(flags), show_expr(e));
        XXTLOG("    %s", ShowType(t0));

        Type *u0 = NewVar(ty);
        type_assign(ty, e, u0, flags);
        XXTLOG("try_assign(%s): after assign:", _show_tflags(flags));
        XXTLOG("    %s", ShowType(u0));
        UnifyX(ty,  Either(ty, u0, NewVar(ty)), t0, true, false);

        e->_type = u0;

        XXTLOG("try_assign(%s): after unify:", _show_tflags(flags));
        XXTLOG(" *  %s", ShowType(t0));
        XXTLOG("    %s", ShowType(u0));
}
...
type_resolve(Ty *ty, Expr const *e)
{
        ...
        case EXPRESSION_SUBSCRIPT:
                ...
                if (TypeType(t0) == TYPE_TYPE) {
                        t0->_type = type_resolve(ty, e->subscript);
                        return t0;
                }
                if (IsTVar(t0) && (e->subscript->type == EXPRESSION_INTEGER)) {
                        ...
                        if (
                             ...
                             || (TypeType(t1) == TYPE_LIST)
                                ...
                                CloneVec(t0->args);
                                avP(t0->args, t1);
                                if (StrictClassOf(t0) == CLASS_GENERATOR) {
                                        avP(t0->args, NIL_TYPE);
                                }
                        }
                        if (t0->type == TYPE_TAG) {
                                t0->type = TYPE_OBJECT;
                        }
                        t0->concrete &= (t1 == NULL || t1->concrete)
                                     && (!IsAliasT(t0)|| (t0->_type != NULL));
                }
        ...
        case EXPRESSION_SLICE:
                ...
                if (IsTVar(t0)) {
                        ...
                        t1->k = IsIntLit(k0) ? (k0->z + !k0->z) : 1;
                        t1->i = IsIntLit(i0) ? i0->z : 0;
                        t1->j = IsIntLit(j0) ? j0->z : (t1->k < 0) ? 0 : INT_MAX;
                        t1->val = t0;
                        t0 = t1;
                }
        ...
        case EXPRESSION_STRING:
                return type_string(ty, e->string);

        case EXPRESSION_BOOLEAN:
                return type_bool(ty, e->boolean);
        }
...
try_coalesce(Ty *ty, Type **t0, Type *t1)
{
        ...
        if (IsIntLit(*t0) && IsIntLit(t1)) {
                return (*t0)->z == t1->z;
        }

        if (IsStrLit(*t0) && IsStrLit(t1)) {
                return s_eq((*t0)->str, t1->str);
        }

        if (IsBoolLit(*t0) && IsBoolLit(t1)) {
                return (*t0)->z == t1->z;
        }
...
type_show(Ty *ty, Type const *t0)
{
        ...
        case TYPE_FUNCTION:
                ...
                if ((vN(t0->constraints) > 0) || (t0->yields != NULL)) {
                        ...
                        dump(
                                &buf,
                                "%s)%s %s=>%s ",
                                TERM(38;2;255;153;187),
                                TERM(0),
                                TERM(38;2;239;145;45),
                                TERM(0)
                        );
                }
        ...
        case TYPE_VARIABLE:
                ...
                if (IsBoundVar(t0)) {
                        ...
                        dump(
                                &buf,
                                "%s{%d}%s",
                                (t0->level >= CurrentLevel) ? TERM(94) : TERM(96),
                                t0->level,
                                TERM(0)
                        );
                }
        ...
        case TYPE_STRING:
                dump(&buf, "%s'%s'%s", TERM(38;2;142;192;124), t0->str, TERM(0));
                break;
        ...
        case TYPE_COMPUTED:
                if (t0->val != NULL) {
                        ...
                        dump(&buf, "%s%s(%s", TERM(92), name, TERM(0));
                        for (int i = 0; i < vN(t0->args); ++i) {
                                if (i > 0) {
                                        dump(&buf, ", ");
                                }
...
MakeConcrete_(Ty *ty, Type *t0, TypeVector *refs, bool variance)
{
        ...
        case TYPE_SUBSCRIPT:
        case TYPE_SLICE:
                t0 = !CanStep(t0) ? t0 : MakeConcrete_(ty, StepVar(t0), refs, variance);
                break;
        ...
        case TYPE_FUNCTION:
                ...
                if (t0->yields != NULL) {
                        t0->yields = MakeConcrete_(ty, t0->yields, refs, variance);
                        t0->concrete &= IsConcrete(t0->yields);
                }
                if (t0->sends != NULL) {
                        t0->sends = MakeConcrete_(ty, t0->sends, refs, variance);
                        t0->concrete &= IsConcrete(t0->sends);
                }
                break;

        case TYPE_TYPE:
                t0->_type = MakeConcrete_(ty, t0->_type, refs, variance);
                t0->concrete &= IsConcrete(t0->_type);
                break;
...
TypeOf2Op(Ty *ty, int op, Type *t0, Type *t1)
{
        ...
        case OP_ADD:
                ...
                case PAIR_OF(CLASS_INT):                 return INT_TYPE;
...
type_special_str(Ty *ty, Expr const *e)
{
        ...
        Type *u0 = NewTuple(
                a0,
                Either(ty, STRING_TYPE, NIL_TYPE),
                Either(ty, INT_TYPE, NIL_TYPE)
        );
        Type *f0 = NewFunction(
                NewArray(
                        Either(
...
IsDisconnected(Type *t0, Type *u0)
{
        ...
        nref += CountRefs(t0->yields, id);
        nref += CountRefs(t0->sends, id);
...
FilterConstraints(Ty *ty, Type *t0)
{
        ...
        for (int i = 0; i < vN(t0->constraints); ++i) {
                Constraint *c = v_(t0->constraints, i);
                if (IsDisconnected(t0, c->t0) || IsDisconnected(t0, c->t1)) {
...
TyTypeVector(Ty *ty, Value const *v)
{
        ...
        default:
                CompileError("expected type vector but got: %s", VSC(v));
                UNREACHABLE();
        }
...
RecordType:
        ...
        if (t0 == NULL) {
Fail:
                CompileError("invalid type spec: %s", VSC(v));
        }
...
types_save(Ty *ty)
{
        ...
        avPv(state.WorkIndex, WorkIndex);
        avPv(state.ToSolve, ToSolve);
        avPv(state.FunStack, FunStack);
        avPv(state.EnvStack, EnvStack);
...
types_restore(Ty *ty, TypeCheckState *state)
{
        ...
        xvPv(WorkIndex, state->WorkIndex);
        xvPv(ToSolve, state->ToSolve);
        xvPv(FunStack, state->FunStack);
        xvPv(EnvStack, state->EnvStack);
}
